...
export default function ProfilePage() {
   ...
   return (
      <div className="max-w-4xl mx-auto space-y-8 pb-12 pt-12 lg:pt-0 px-2 lg:px-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
         {/* Profile Hero Card */}
               ...
               <div className="text-center md:text-left flex-1 space-y-4">
                  <div>
                     <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mb-2">
                        <h2 className="text-4xl md:text-5xl font-black tracking-tight leading-none mb-0.5">{user.name}</h2>
                        {user.role === 'admin' ? (
                           <div className="relative group mt-2 shrink-0">
                              <div className="absolute -inset-1.5   bg-gradient-to-r from-amber-500 via-rose-500 to-fuchsia-500 rounded-full blur opacity-40 group-hover:opacity-75 transition duration-500"></div>
                              <div className="relative h-8 px-7 mt-1 bg-slate-950/80 backdrop-blur-sm rounded-full border border-white/10 flex items-center justify-center shadow-2xl">
                                 <span className="bg-clip-text text-transparent bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-500 font-black tracking-[0.2em] uppercase text-[10px] drop-shadow-sm pt-[2px]">
                                    {user.role}
                                 </span>
                              </div>
                           </div>
                        ) : (
                           <Badge variant="member" className="h-6 px-4 text-xs font-bold uppercase tracking-widest ring-4 ring-white/5 shrink-0">
                              {user.role}
                           </Badge>
                        )}
                     </div>
